...
- After deduplication and one consolidated confirmation, put every genuinely missing item in one `create-items` manifest with a unique `clientId`. Include already-decided collection membership and metadata in the create payload instead of immediately issuing a second update. Use `--expect-count N --yes`; the CLI splits batches at 50 and performs one consolidated readback.
